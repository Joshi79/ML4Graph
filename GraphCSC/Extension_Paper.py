...
def train(train_data, centrality_measures, context=True, centrality=True):
    G = train_data[0]
    ...
    minibatch = EdgeMinibatchIterator(
        G, id_map, placeholders, centrality_measures, context_pairs = context_pairs_walk, batch_size=batch_size, max_degree=max_degree, centrality=centrality
    )
    ...
    sampler = NeighborSampler(adj_info, centrality_measures)
    layer_infos = [
    ...
    with open("train_nodes_bridge.txt", "w") as f:
        for node in train_nodes:
            f.write(f"{node}\n")

    with open("test_nodes_bridge.txt", "w") as f:
...
def main():
    ...
    centrality_dict = load_centrality_measures(directory, "ppi-bridge_strength_normalized.json")
    train_data = load_data(train_prefix, directory, load_walks=True, degree=False)
    print("Data loaded.")
    train(train_data, centrality_dict, context=True, centrality=False)
