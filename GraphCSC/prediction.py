...
class BipartiteEdgePredLayer(Layer):
    ...
    def centrality_score(self, emb, reuse=False):
        """
        A small MLP or linear layer that maps from embedding -> scalar score.
        ...
        with tf.variable_scope("centrality_nn", reuse=reuse):
            ...
            score = tf.layers.dense(hidden, 1, activation=None,
                                    name='output_layer')
            score = tf.squeeze(score, axis=1)  # shape [batch_size]
        return score
    ...
    def ranking_loss(self, inputs1, inputs2):
        ...
        loss_c = -tf.reduce_sum(tf.log(ranking_prob + 1e-10))  # +1e-10 for numerical stability

        return loss_c

    def total_lost(self, inputs1, inputs2, neg_samples, hard_neg_samples=None):
        Lg_losss = self._xent_loss(inputs1, inputs2, neg_samples, hard_neg_samples=None)

        Lc_loss = loss_c = self.ranking_loss(inputs1, inputs2)  # from code above

        lambda_c = 0.1
        total_loss = Lg_losss + lambda_c * Lc_loss
        return total_loss
